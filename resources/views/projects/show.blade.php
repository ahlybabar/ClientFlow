...
<nav class="flex items-center gap-2 text-sm text-[#64748B]">
    ...
    <span>/</span>
    <a href="{{ route('projects.index') }}" class="hover:text-[#4F46E5]">Projects</a>
    <span>/</span>
    <a href="{{ route('clients.show', 1) }}" class="hover:text-[#4F46E5]">Acme Corp</a>
    <span>/</span>
...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <span class="px-3 py-1.5 rounded-full text-sm font-medium bg-[#EEF2FF] text-[#4F46E5]">In Progress</span>
            {{-- Project Health --}}
    ...
    <div x-data="{ tab: 'tasks' }" class="space-y-6">
        <div class="border-b border-[#E2E8F0] overflow-x-auto">
            <nav class="flex gap-6 min-w-max">
                ...
                <button @click="tab = '{{ $key }}'" :class="tab === '{{ $key }}' ? 'border-[#4F46E5] text-[#4F46E5]' : 'border-transparent text-[#64748B] hover:text-[#0F172A]'"
        ...
        <div x-show="tab === 'overview'" class="space-y-6">
            ...
            <div class="bg-white rounded-xl p-6 border border-[#E2E8F0] shadow-sm">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
                    ...
                    <div class="p-3 rounded-lg bg-[#F8FAFC] border border-[#E2E8F0] text-center">
                        <p class="text-2xl font-bold text-[#4F46E5]">6 days</p>
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <div class="h-2 bg-[#E2E8F0] rounded-full overflow-hidden">
                            <div class="h-full bg-[#4F46E5] rounded-full progress-animated" style="width: 72%"></div>
                        </div>
                    </div>
                    <div>
                        <div class="flex items-center justify-between text-sm mb-1.5">
                    ...
                    <div class="h-48 flex items-end gap-2">
                        ...
                        <div class="flex-1 flex flex-col items-center gap-1">
                            <div class="w-full bg-[#E2E8F0] rounded-t overflow-hidden" style="height: 120px;">
                                <div class="h-full bg-[#4F46E5] rounded-t progress-animated" style="width: 100%; height: {{ $m['pct'] }}%; margin-top: auto;"></div>
                            </div>
                    ...
                    <div class="space-y-3 sortable-list">
                        ...
                        <div class="bg-white rounded-lg p-4 border border-[#E2E8F0] shadow-sm cursor-move hover:shadow-md transition-all">
                            ...
                            <div class="mt-2 flex items-center gap-2 flex-wrap">
                                <span class="w-6 h-6 rounded-full bg-[#4F46E5] text-white text-xs flex items-center justify-center">JD</span>
                                <span class="text-xs px-2 py-0.5 rounded font-medium {{ $t['priority'] === 'Critical' ? 'bg-[#FEF2F2] text-[#EF4444]' : ($t['priority'] === 'High' ? 'bg-[#EEF2FF] text-[#4F46E5]' : 'bg-[#F1F5F9] text-[#64748B]') }}">{{ $t['priority'] }}</span>
                            </div>
                        </div>
                        @endforeach
                    </div>
        ...
        <div x-show="tab === 'invoices'" class="bg-white rounded-xl p-6 border border-[#E2E8F0]">
            ...
            <a href="{{ route('invoices.index') }}" class="inline-block mt-2 text-sm font-medium text-[#4F46E5] hover:underline">View invoices →</a>
        </div>
        ...
        <div x-show="tab === 'files'" class="bg-white rounded-xl p-6 border border-[#E2E8F0]">
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-3 p-3 rounded-lg border border-[#E2E8F0] hover:bg-[#F8FAFC]">
                    <div class="w-10 h-10 rounded-lg bg-[#EEF2FF] flex items-center justify-center text-[#4F46E5]">
            ...
            <button class="mt-4 text-sm font-medium text-[#4F46E5] hover:underline">+ Upload file</button>
        </div>
        ...
        <div x-show="tab === 'team'" class="bg-[var(--color-card)] rounded-xl p-6 border border-[var(--color-border)] shadow-sm transition-colors">
            <div class="flex items-center justify-between mb-6">
                ...
                <button onclick="window.toast?.('Manage Team modal opened', 'info')" class="inline-flex items-center gap-2 px-4 py-2 bg-[#4F46E5] text-white rounded-lg hover:bg-[#4338CA] transition-colors text-sm font-medium">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div class="flex items-center justify-between p-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] hover:border-[var(--color-text-muted)] transition-colors group">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-[#4F46E5] text-white flex items-center justify-center font-medium text-sm">{{ substr($member['name'], 0, 2) }}</div>
                        <div>
