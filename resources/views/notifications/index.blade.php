...
<nav class="flex items-center gap-2 text-sm text-[#64748B]">
    ...
    <span>/</span>
    <span class="text-[#0F172A] font-medium">Notifications</span>
</nav>
@endsection
...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-[#0F172A]">Notifications</h1>
        <button class="text-sm font-medium text-[#4F46E5] hover:underline">Mark all as read</button>
    </div>
    ...
    <div class="bg-white rounded-xl border border-[#E2E8F0] overflow-hidden">
        ...
        <a href="#" class="flex gap-4 px-6 py-4 hover:bg-[#F8FAFC] transition-colors {{ $n['unread'] ? 'bg-[#EEF2FF]/50' : '' }} border-b border-[#E2E8F0] last:border-0">
            <div class="w-10 h-10 rounded-full shrink-0 flex items-center justify-center {{ $n['type'] === 'task' ? 'bg-[#EEF2FF] text-[#4F46E5]' : ($n['type'] === 'invoice' ? 'bg-[#ECFDF5] text-[#22C55E]' : 'bg-[#FEF3C7] text-[#F59E0B]') }}">
            ...
            <div class="flex-1 min-w-0">
                <p class="font-medium text-[#0F172A]">{{ $n['title'] }}</p>
                <p class="text-sm text-[#64748B] mt-0.5">{{ $n['body'] }}</p>
                <p class="text-xs text-[#64748B] mt-1">{{ $n['time'] }}</p>
            </div>
            @if($n['unread'])<span class="w-2 h-2 rounded-full bg-[#4F46E5] dark:bg-[#6366F1] shrink-0 mt-2"></span>@endif
        </a>
        @endforeach
    </div>
</div>
